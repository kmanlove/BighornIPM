                            ...
                            joint.posterior.coda, posterior.names, intro.cost)
{
  ...
  # samples.to.draw = numerical range of chain steps that are outside the burn-in period 
  #    in joint.posterior.coda
  ...
  if(current.state == "healthy"){
    repros <- c(0, rep((post.draw$beta.wean.1.2 * sex.ratio), 1), 
                rep((post.draw$beta.wean.1.3  * sex.ratio), 5), 
                rep((post.draw$beta.wean.1.4  * sex.ratio), 6), 
                rep((post.draw$beta.wean.1.5  * sex.ratio), 6))    
    survs <- c(1, rep(post.draw$beta.adsurv.1.2, 1), 
               rep(post.draw$beta.adsurv.1.3, 5), 
               rep(post.draw$beta.adsurv.1.4, 6), 
    ...
    survs <- c(1, rep(post.draw$beta.adsurv.2.2, 1), 
               rep(post.draw$beta.adsurv.2.3, 5), 
               rep(post.draw$beta.adsurv.2.4, 6), 
    ...
    survs <- c(1, rep(post.draw$beta.adsurv.1.2, 1), 
               rep(post.draw$beta.adsurv.1.3, 5), 
               rep(post.draw$beta.adsurv.1.4, 6), 
               rep(post.draw$beta.adsurv.1.5, 5)) * (1 - intro.cost)
    leslie <- rbind(repros, cbind(diag(c(survs)), rep(0, length(survs)))) 
  }
  return(leslie)
}
